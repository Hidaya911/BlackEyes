...
interface Result { record_type: 'product' | 'customer_invoice' | 'vendor_invoice'; id: number; title: string; description: string; amount: number; status: string; date: string | null; similarity_score: number; tags: string[] }
interface Response { query: string; results: Result[]; searched_count: number; limit: number }
...
export function SmartSearch({ isAdmin = false }: { isAdmin?: boolean }) {
  ...
  async function search(text = query, scope = type) {
    if (text.trim().length < 2) { setError('Enter at least two characters.'); return; }
    request.current?.abort();
    const controller = new AbortController(); request.current = controller;
    setQuery(text); setLoading(true); setError(''); setData(null); setTag('');
    try {
      const response = await fetch(`/api/search/semantic?${new URLSearchParams({ q: text.trim(), record_type: scope })}`, { signal: controller.signal, credentials: 'same-origin' });
  ...
  return <div className="smart-search">
    ...
    <section className="smart-results" aria-busy={loading}>
      <div className="smart-toolbar"><div><h3>Your discoveries</h3><p aria-live="polite">{data ? `${results.length} matches · ${data.searched_count} records searched for “${data.query}”` : 'One search across your workspace'}</p></div>
      ...
      {loading ? <div className="smart-empty" role="status"><FaMagic /><h4>Finding connections…</h4><p>The first search may take longer while the local model loads.</p></div> : data && results.length ? <>
        <table className="smart-table"><caption className="visually-hidden">Semantic search results, ordered by relevance</caption><thead><tr><th>Record</th><th>Suggested tags</th><th>Date</th><th>Amount</th><th>Relevance</th></tr></thead><tbody>
          {results.map(result => <tr key={`${result.record_type}-${result.id}`}>
            ...
            <td data-label="Relevance"><span className="smart-score" title="Semantic similarity, not a probability">{result.similarity_score.toFixed(2)}</span><small>similarity</small></td>
          </tr>)}
        </tbody></table>
        <footer>Ordered by semantic similarity. Tags are automatic suggestions.{data.results.length === data.limit && ' Showing the top 30 matches. Refine your search to narrow the results.'}</footer>
