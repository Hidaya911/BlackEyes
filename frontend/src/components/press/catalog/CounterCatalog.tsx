...
export function CounterCatalog({ products, wholesale, disabled, onAdd }: Props) {
  ...
  return <div className="counter-catalog"><div className="counter-catalog-banner"><span className="counter-catalog-icon"><FaStore /></span><div><small>BLACKEYES / SHOP THE PRINT</small><h4>Your counter catalog</h4><p>Pick a product. Add the details. Build the order.</p></div><span className="counter-price-tag">{wholesale ? 'Wholesale prices' : 'Customer prices'}</span></div><label className="counter-catalog-search"><FaSearch /><input type="search" aria-label="Search counter catalog" placeholder="Search products or descriptions…" value={query} onChange={event => setQuery(event.target.value)} /></label><div className="counter-catalog-grid" aria-busy={disabled}>{visible.map(product => <button type="button" key={product.product_id} className="counter-catalog-card" disabled={disabled} aria-label={`Customize ${product.name}`} onClick={() => setSelected(product)}><div className="counter-catalog-image"><ProductImage product={product} /><span><FaPlus /></span></div><div className="counter-catalog-card-info"><h5>{product.name}</h5><p>{product.description || 'Made to your specifications'}</p><div><strong>{disabled ? '…' : product.price == null ? 'Set agreed price' : customerMoney(product.price)}</strong><small>{product.price_kind === 'wholesale' ? 'Wholesale' : product.special_price ? 'Special' : 'Retail'} / unit</small></div><span>Choose & customize →</span></div></button>)}</div>{!visible.length && <p className="counter-catalog-empty">{products.length ? 'No products match your search.' : 'No catalog products available. You can add a custom service below.'}</p>}{selected && !disabled && <ConfigureItem product={selected} onClose={() => setSelected(null)} onAdd={onAdd} />}</div>;
}
