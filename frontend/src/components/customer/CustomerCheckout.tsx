...
export function CustomerCheckout({ cart, products, phone, onChange, onClose, onPlaced, onRefresh }: Props) {
  ...
  const lines = cart.map(line => ({ ...line, product: products.find(p => p.product_id === line.product_id) }));
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    for (const line of lines) {
      const failure = designError(line.quantity, line.designs ?? []);
      if (failure) { setStep(0); setError(`${line.product?.name}: ${failure}`); return; }
    }
    const failure = attachmentError(cart);
    if (failure) { setStep(0); setError(failure); return; }
    if (step < 2) { setStep(step + 1); return; }
    setBusy(true);
    try {
      const items = await Promise.all(cart.map(async line => ({
  ...
  return <Modal show onHide={() => !busy && onClose()} size="lg" centered scrollable backdrop={busy ? 'static' : true} keyboard={!busy} contentClassName="customer-modal" aria-labelledby="checkout-title"><Modal.Header closeButton={!busy}><Modal.Title id="checkout-title">Your next great print.</Modal.Title></Modal.Header><form onSubmit={submit}><Modal.Body>
    <ol className="customer-checkout-steps">{['Customize & design', 'Contact & collection', 'Review'].map((label, index) => <li key={label} className={index <= step ? 'is-active' : ''}><b>{index < step ? <FaCheck /> : index + 1}</b>{label}</li>)}</ol>
    {!cart.length ? <div className="customer-empty"><h3>Your basket is empty.</h3><p>Add a product to start your print project.</p></div> : <fieldset disabled={busy}>
      {step === 0 && <div className="customer-cart-lines">{lines.map(line => <article key={line.product_id}><div className="customer-cart-line-heading"><div><h3>{line.product?.name || 'Unavailable product'}</h3><small>{line.product ? `${customerMoney(line.product.price)} per unit` : 'Remove this item to continue.'}</small></div><button className="customer-icon-button" type="button" aria-label={`Remove ${line.product?.name || 'unavailable product'}`} onClick={() => onChange(cart.filter(item => item.product_id !== line.product_id))}><FaTrash /></button></div>
        <div className="customer-cart-line-fields"><label className="customer-field"><span>Quantity</span><input className="form-control" type="number" min={1} max={100000} step={1} required value={line.quantity || ''} onChange={e => { const quantity = Number(e.target.value); update(line.product_id, { quantity, designs: resizeDesigns(line.designs, quantity) }); }} /></label><label className="customer-field"><span>Print details</span><input className="form-control" maxLength={2000} value={line.specifications} placeholder="Size, colour, finish…" onChange={e => update(line.product_id, { specifications: e.target.value })} /></label><strong>{customerMoney((line.product?.price ?? 0) * line.quantity)}</strong></div>
        <ItemDesignEditor quantity={line.quantity} designs={line.designs?.length ? line.designs : [emptyDesign(line.quantity)]} onChange={designs => update(line.product_id, { designs })} />
      </article>)}</div>}
